...
async function throttled(items, n, fn) {
...
export async function fetchMacroEvidence({ finnhubKey, timeoutMs = 5000 } = {}) {
  ...
  const [batch, news] = await Promise.all([
    batchYahoo(symbols, timeoutMs),
    generalNews(finnhubKey, timeoutMs),
  ]);

  const missing = MACRO_UNIVERSE.filter(m => !batch.quotes[m.yahoo]);
  const fallback = await throttled(missing, 2, m => quoteYahoo(m.yahoo, timeoutMs));
  ...
  const levels = MACRO_UNIVERSE.map(m => {
    ...
    return f.ok
      ? { ...m, ok: true, via: 'chart', ...f.v }
      : { ...m, ok: false, error: `${f.e?.message || 'unavailable'}${batch.error ? ` (batch: ${batch.error})` : ''}` };
  });
...
export async function macroEvidenceBlock({ finnhubKey, ttlMs = 5 * 60000, timeoutMs = 4000 } = {}) {
  ...
  const ev = await fetchMacroEvidence({ finnhubKey, timeoutMs });
  const okCount = ev.levels.filter(l => l.ok).length;
