...
// ── GDELT GEO ──
export const EVENTS_QUERY = '(airstrike OR airstrikes OR missile OR shelling OR "drone attack" OR "drone strike" OR clashes OR bombardment)';
...
export async function fetchEvents({ timeoutMs = 12000, maxpoints = 250 } = {}) {
  const p = new URLSearchParams({ query: EVENTS_QUERY, mode: 'PointData', format: 'GeoJSON', timespan: '24H', maxpoints: String(maxpoints) });
  const r = await timedFetch(`${GDELT_GEO}?${p}`, timeoutMs, { headers: { Accept: 'application/json' } });
